...
@st.cache_data 
def load_data1():
   return pd.read_csv("models_saved\data_set_f\\dataset_analyse.csv")

@st.cache_data 
def load_data2():
   return pd.read_csv("models_saved\data_set_f\dataset_to_prepro.csv")
 
@st.cache_data 
def load_data3():
   return pd.read_csv("models_saved\data_set_f\\dataset_final.csv") 
...
if page == pages[3] : 
  ...
  with col1:
      # st.header("Choisir la distribution")
      ...
      if val_original:
        ...
        st.pyplot(plt)
  with col2 :
    # st.header("Données après préprocessing")
...
# Deuxième colonne avec case à cocher
  ...
  with col2 :
    # st.header("Nouvelle tranche horaire")
  ...
  col1, col2 = st.columns(2)
  with col1 :
    # st.header("Mode de déplacements avant le préprocessing")
  ...
  with col2 :   
      # st.header("Mode de déplacements après le préprocessing")
  ...
  with tab1 :
    # st.header("Système de sécurité avant le preprocessing")
  ...
  with tab2 :
    # st.header("Système de sécurité après le preprocessing")
...
if page == pages[4] : 
  ...
  models = {
    ...
    "model extra trees classifier" : joblib.load('models_saved\extra_tree_classifier_model.joblib'),
    ...
    "model random forest" : joblib.load('projet_av\Codes Final\models_saved\rf_model.joblib')
    }
