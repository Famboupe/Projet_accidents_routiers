...
@st.cache_data 
def load_data1():
   return pd.read_csv("dataset_saved\\dataset_analyse.csv")

@st.cache_data 
def load_data2():
   return pd.read_csv("dataset_saved\dataset_to_prepro.csv")
 
@st.cache_data 
def load_data3():
   return pd.read_csv("dataset_saved\dataset_final.csv") 
...
if page == pages[2] : 
  ...
  # Assurez-vous que les valeurs sont des chaînes de caractères avant d'utiliser replace
  df_temp["lat"] = df_temp["lat"].astype(str).apply(lambda x: x.replace(',', '.'))
  df_temp["long"] = df_temp["long"].astype(str).apply(lambda x: x.replace(',', '.'))

  # Convertissez ensuite les colonnes en float
...
if page == pages[3] : 
  ...
  # Ajouter des onglets
  col1, col2 = st.tabs(["Données originales", "Données après remplacement"])

  # Première colonne avec case à cocher
  with col1:
      st.header("Comptage des départements 2A et 2B")
...
# Deuxième colonne avec case à cocher
  with col2:
      st.header("Comptage après remplacement")
  ...
  with tab1 :
    st.header("Comptage Avant remplacement")
  ...
  col1, col2 = st.tabs(["Variables originales", "Nouvelles varibale"])
  df_h = df_ana.copy()
  with col1 :
    st.header("Tranche horaire originale")
    val_original = st.checkbox("Variables originales", key = "orig")
    if val_original:
      df_h = df_ana.copy()
      st.write(df_h['hrmn'].value_counts())
  
  with col2 :
    st.header("Nouvelles tranche horaire")
    val_replaced = st.checkbox("nouvelles variables", key ="repl")
    if val_replaced:
      df_h = df_ana.copy()
  ...
  tab1, tab2 = st.tabs(["Données originales", "Données après remplacement"])
  with tab1 :
    st.header("Tranche d'âge originale")
  ...
  with tab2 :
    st.header("Nouvelle tranche d'âge")
  ...
  col1, col2 = st.columns(2)
  with col1 :
    st.header("Catégories véhicules")
    val_or = st.checkbox("Modes de déplacements", key = "or")
    if val_or:
      df_depl = df_ana.copy()
      st.write(df_depl['catv'].value_counts())
  with col2 :   
      st.header("Distribution des accidents")
      val_re = st.checkbox("Afficher la distribution des accidents", key="re")
      if val_re:
        fig, ax = plt.subplots(figsize=(5, 5))
        ax.hist(df_model['catv'], color='#f27750', label='catv')
        ax.set_xlabel("Mode de déplacement")
        ax.set_ylabel("Nombre d'accidents")
        ax.set_xticks([0, 1, 2, 3, 4, 5, 6])
        ax.set_xticklabels(['Marche', 'EDPM', "Vélo", "2RM", "VT", "VU", "Voiture"])
        ax.set_title('Distribution des accidents par mode de déplacement')
        st.pyplot(fig)
  ...
  tab1, tab2 = st.tabs(["Avant regroupement", "Aprés regroupement"])
  with tab1 :
    st.header("Système de sécurité avant regroupement")
  ...
  with tab2 :
    st.header("Système de sécurité après regroupement")
    Syst_ap = st.checkbox("Nouvelles originales", key = "Syst")
    if Syst_ap:
...
if page == pages[4] : 
  ...
  models = {
    # "model adaboost" : joblib.load(r'C:\Users\belmo\OneDrive\Bureau\DATASCINETIST\PROJECT\projet_av\Codes Final\models_saved\best_estimator_ada_model.joblib'),
    "model extra trees classifier" : joblib.load(r'C:\Users\belmo\OneDrive\Bureau\DATASCINETIST\PROJECT\projet_av\Codes Final\models_saved\extra_tree_classifier_model.joblib'),
    "model xgbc" : joblib.load(r'C:\Users\belmo\OneDrive\Bureau\DATASCINETIST\PROJECT\projet_av\Codes Final\models_saved\GBClassifier_model.joblib'),
    # "model régression logistique" : joblib.load(r'C:\Users\belmo\OneDrive\Bureau\DATASCINETIST\PROJECT\projet_av\Codes Final\models_saved\log_reg_model.joblib'),
    "model lgbm" : joblib.load(r'C:\Users\belmo\OneDrive\Bureau\DATASCINETIST\PROJECT\projet_av\Codes Final\models_saved\LGBMClassifier_model.joblib'),
    # "model random forest" : joblib.load(r'C:\Users\belmo\OneDrive\Bureau\DATASCINETIST\PROJECT\projet_av\Codes Final\models_saved\rf_model.joblib'),
    "model knn" : joblib.load(r'C:\Users\belmo\OneDrive\Bureau\DATASCINETIST\PROJECT\projet_av\Codes Final\models_saved\KNeighborsClassifier_model.joblib')
    }
